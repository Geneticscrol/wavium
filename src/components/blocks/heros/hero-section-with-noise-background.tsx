...
import { downloadWebsiteAsZip } from "@/lib/download-website";
...
export function HeroSectionWithNoiseBackground() {
  ...
  const handleDownloadWebsite = async () => {
    ...
    try {
      const result = await downloadWebsiteAsZip(
      ...
      if (result.success) {
        setDownloadProgress({
          current: result.filesDownloaded,
          total: result.filesDownloaded,
          status: `Successfully downloaded ${result.filesDownloaded} files!`,
        });
      } else {
          ...
          {isDownloading && downloadProgress.status && (
            <div className="text-center">
              ...
              {downloadProgress.total > 0 && (
                <div className="mt-2 w-64 bg-neutral-200 rounded-full h-2 dark:bg-neutral-700">
                  <div
